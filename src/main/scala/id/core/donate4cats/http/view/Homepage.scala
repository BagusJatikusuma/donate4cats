  ...
  private def header(): TypedTag[String] =
    ...
    div(
      ...
      div(
        div(
          a(
            href := s"https://circlestreamid.kinde.com/oauth2/auth?response_type=code&client_id=$clientId&redirect_uri=$redirectUrl&scope=openid+profile+email&state=$token",
            button(
